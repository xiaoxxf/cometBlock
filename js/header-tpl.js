...
$(document).ready(function(){
    $(".show-alert-inform-top li a").click(function(){
        order = $(".show-alert-inform-top li a").index(this);//获取点击之后返回当前a标签index的值
        $(".inform" + order).show().siblings("div").hide();//显示class中con加上返回值所对应的DIV
   			$(".show-alert-inform-bottom").css("display","block");
				...
				if (order == 1) {
					...
					if ( !comment_tpl_flag ) {
						...
						var uri = 'news/getMessage?userId=' + userId + '&userPwd=' + userinfo.userPwd +
											'&currentPage=' + noticeCurrentPage + '&pageSize=' + noticePageSize + '&type=' + notification_type
				...
				// 审核通过4 + 驳回5
					...
					doJavaGet(uri,function(result){
						if (result.datas.length !== 0) {
							var tpl = document.getElementById('topic_message_tpl').innerHTML;
							var content = template(tpl, {list: result.datas});
							$('.topic-message').html('')
							$('.topic-message').append(content)
						}
					})
...
function countUnreadMessage(){
	if (userId) {
		notification_type = [1,2,3,4,5,6]
		var uri = 'news/getMessage?userId=' + userId + '&userPwd=' + userinfo.userPwd + '&currentPage=1'  + '&pageSize=1' + '&type=' + notification_type
		doJavaGet(uri,function(result){
			if (result.code == 0) {
